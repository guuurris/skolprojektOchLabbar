...
<?php
	...
	function changeCategorySubscriptionCheck()
	{
		$out = null;
		if(isset($_REQUEST['submit']) )
		{
			$out['prenumerate'] = "satt";
		}
		
		if(!empty($_REQUEST['checkedCategory']))
		{
			$out['checkedCategory'] = $_REQUEST['checkedCategory'];
	...
	function getFoodValues()
	{
		...
		isset($_POST['procedure']) and isset($_POST['description']) 	)
		{
			...
			$food_values['name'] = htmlspecialchars($_POST['namn_ratt']);
			$food_values['difficulty'] = $_POST['Svarighet'];
			$food_values['cookingTime'] = $_POST['Tillagningstid'];
			$food_values['description'] = htmlspecialchars($_POST['description']);
			$food_values['procedure'] = htmlspecialchars($_POST['procedure']);
